...
sed -E "s/^import [^;]*;?$//; s/^export (const|function|let|class|var)/\1/; s/^export \{[^}]*\};?$//" \
  ...
  js/augusta-elevation.js js/augusta-world-data.js js/standrews-private.js \
  js/standrews-osm.js js/standrews-elevation.js js/standrews-world-data.js \
  js/world.js js/terrain.js > /tmp/gspro-core.js
