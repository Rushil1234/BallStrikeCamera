import { HOLES } from '../js/holes.js';
import { layoutIslandCourse } from '../js/world.js';

const world = layoutIslandCourse(HOLES);
const failures = [];
...
for (let i = 0; i < world.holes.length; i++) {
  for (let j = i + 1; j < world.holes.length; j++) {
    ...
    for (let ai = 0; ai < aHole.path.length - 1; ai++) {
      for (let bi = 0; bi < bHole.path.length - 1; bi++) {
        if (segmentsIntersect(aHole.path[ai], aHole.path[ai + 1], bHole.path[bi], bHole.path[bi + 1])) {
          failures.push(`Fairway centerlines cross: hole ${aHole.id} segment ${ai + 1} with hole ${bHole.id} segment ${bi + 1}`);
        }
    ...
    if (minDistance < minFairwayGap) {
      failures.push(`Fairway corridors too close: holes ${aHole.id}/${bHole.id} are ${minDistance.toFixed(1)}m apart, need ${minFairwayGap}m`);
    }
  }
}
...
for (const hole of world.holes) {
  ...
  if (minPathClearance < 0) {
    failures.push(`Hole ${hole.id} fairway overlaps shared lake by ${Math.abs(minPathClearance).toFixed(1)}m`);
  }
  ...
  for (const [label, p, pad] of featureChecks) {
    for (const w of world.water || []) {
      ...
      if (clear < 0) {
        failures.push(`Hole ${hole.id} ${label} is inside a shared lake by ${Math.abs(clear).toFixed(1)}m`);
      }
    }
  }
}
...
console.log(`Layout OK: ${world.holes.length} holes, ${world.water.length} shared lakes, fairways separated, no shared-lake overlaps.`);
